...
def fair_metric(y, sens, output, idx):
    ...
    """parity = abs(
        val_y[idx_s0].mean() - val_y[idx_s1].mean()
    )"""
...
# GraphTransformer position encoding (UNUSED)
...
def adjacency_positional_encoding(g, pos_enc_dim):
    eignvalue, eignvector = sp.linalg.eigsh(g, which='SM', k=pos_enc_dim)
...
def re_features(adj, features, K): 
    ...
    for i in range(K): # 0 -> K-1
        ...
        if isinstance(adj, torch.Tensor):
            x = torch.mm(adj, x) # torch_sparse.matmul(adj, x)
        else:
            x = adj @ x
...
def calculDS(sens):
    ...
    I_pos = (s > 0).float()    
    I_neg = (s <= 0).float()   
    I_pos_norm = I_pos / I_pos.sum()
    I_neg_norm = I_neg / I_neg.sum()
    delta_s = I_pos_norm - I_neg_norm
    return delta_s
...
def get_only_homo_edges(adj, sens, keepratio, seed=22):
    ...
    edge_index_direct = edge_index[:,(edge_index[0]-edge_index[1])<=0]
    
    print('total edge num:',edge_index_direct.shape[1])
    is_same_attr = (sens[edge_index_direct[0]]==sens[edge_index_direct[1]])
    
    # print('same attribute edge num:',is_same_attr.shape[1])
    is_same_indices = torch.nonzero(is_same_attr).squeeze() 
    print('same attribute edge num:',is_same_indices.shape[0])
    
    is_same_rand_arg = torch.randperm(is_same_indices.size(0)) 
    keep_same_indices=None
    if keepratio<1.0:
        keep_same_indices = is_same_indices[is_same_rand_arg[:int(keepratio * is_same_indices.size(0))]] 
    else:
...
def get_only_hetero_edges(adj, sens, keepratio, seed=22):
    ...
    is_diff_rand_arg = torch.randperm(is_diff_indices.size(0)) 
    
    keep_diff_indices=None
    if keepratio<1.0:
        keep_diff_indices = is_diff_indices[is_diff_rand_arg[:int(keepratio * is_diff_indices.size(0))]] 
    else:
...
def get_same_sens_sub_complete_graph(adj, sens, subnum, args):
    ...
    try:
        ...
        edge_index = new_graph.all_edges()
        
        new_adj = torch.tensor(
            row = edge_index[0],
            col = edge_index[1]
        ).to_sparse()# new_graph.adj()
        torch.save(new_adj, filepath)
        # lpe=eignvector
        time_cost = time.time()-homo_start
        print('create adj time is {:.3f}'.format(time_cost))
...
def get_diff_sens_complete_graph(adj, sens):
    ...
    for keys in list(itertools.permutations(uni_sens, 2)):
        ...
        node_indices1 = (sens==keys[1]).nonzero().squeeze() 
...
def get_same_label_and_sens_num_nodeid(labels,sens,choosed_labels=[0,1]):
    ...
    for group in label_groups: # 每类
        mask_A0 = torch.where(sens[group] == 0)[0]  
        mask_A1 = torch.where(sens[group] == 1)[0]  
    ...
    for group in label_groups: 
        group_size = min_num  
        mask_A0 = torch.where(sens == 0)[0] 
        mask_A1 = torch.where(sens == 1)[0] 
